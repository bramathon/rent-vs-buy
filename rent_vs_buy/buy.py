...
def buy(
    ...
    interest_cost, principal_cost, balance = mortgage_balance(
        years,
        mortgage_amount,
        parameters.mortgage_rate,
        parameters.mortgage_term
    )
    mortgage_payment_total = interest_cost + principal_cost
    # recurring costs
    property_tax_total = (
        parameters.property_tax_rate * parameters.price * (1 / r) * (np.exp(r * years) - np.exp(r * 0))
    )
    maintenance_total = (
        parameters.maintenance * (1 / r) * (np.exp(r * years) - np.exp(r * 0))
    )
    insurance_total = (
